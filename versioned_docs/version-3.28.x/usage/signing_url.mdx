---
description: Learn about how to sign imgproxy URLs to prevent an attacker from modifying them
---
...
Read our [Configuration](../configuration/options.mdx#url-signature) guide to learn more ways of setting keys and salts.
...
* Take the part of the path after the signature:
  * For [processing URLs](./processing.mdx): `/%processing_options/%encoded_url.%extension`, `/%processing_options/plain/%plain_url@%extension`, or `/%processing_options/enc/%encrypted_url.%extension`
  * For [info URLs](./getting_info.mdx): `/%info_options/%encoded_url`, `/%info_options/plain/%plain_url`, or `/%info_options/enc/%encrypted_url`
