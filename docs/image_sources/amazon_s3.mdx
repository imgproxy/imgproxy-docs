...
6. _(optional)_ [Restrict which buckets imgproxy can access](#restricting-bucket-access) with `IMGPROXY_S3_ALLOWED_BUCKETS` and `IMGPROXY_S3_DENIED_BUCKETS`.
7. _(optional)_ Specify [S3 access points](#access-points) with `IMGPROXY_S3_ACCESS_POINTS`.
...
imgproxy supports accessing S3 buckets through [access points](https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-points.html). To enable this feature, set the `IMGPROXY_S3_ACCESS_POINTS` environment variable to a semicolon-separated list of `name=arn` pairs, where `name` is an arbitrary access point name and `arn` is the access point ARN. For example:

```
IMGPROXY_S3_ACCESS_POINTS="my-access-point1=arn:aws:s3::123456789012:accesspoint/my-access-point1;my-access-point2=arn:aws:s3::123456789012:accesspoint/my-access-point2"
```

When the name is used as a bucket name in a `s3://...` source URL, imgproxy will use the corresponding ARN to access the bucket. For example, with the above configuration, the following source URL will access the bucket through `arn:aws:s3::123456789012:accesspoint/my-access-point1`:

```
s3://my-access-point1/my-image.jpg
```
