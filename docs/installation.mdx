...
# Replace "v4" with the imgproxy version you want to deploy
docker pull ghcr.io/imgproxy/imgproxy:v4-amd64
docker tag ghcr.io/imgproxy/imgproxy:v4-amd64 registry.heroku.com/your-application/web
...
```bash
sudo dpkg -i imgproxy-4.0.0.arm64.deb
```

Where `imgproxy-4.0.0.arm64.deb` is the name of the DEB package you've exported.
...
```bash
sudo rpm -i imgproxy-4.0.0.arm64.rpm
```

Where `imgproxy-4.0.0.arm64.rpm` is the name of the RPM package you've exported.
...
```bash
tar -xzf imgproxy-4.0.0.arm64.tar.gz -C /opt
```

Where:
* `imgproxy-4.0.0.arm64.tar.gz` is the name of the TAR package you've exported.
...
mkdir imgproxy
cd imgproxy
curl -Ls https://github.com/imgproxy/imgproxy/archive/v4.0.0.tar.gz \
  | tar -xz --strip-components 1 -C .
```
...
CGO_LDFLAGS_ALLOW="-s|-w" \
  go build -o /usr/local/bin/imgproxy ./cli
```
