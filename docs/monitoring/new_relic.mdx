...
* `cache/hits`: the number of cache hits (sent only when greater than 0)
* `cache/misses`: the number of cache misses (sent only when greater than 0)
* `src_cache/hits`: the number of source image cache hits (sent only when greater than 0)
* `src_cache/misses`: the number of source image cache misses (sent only when greater than 0)
