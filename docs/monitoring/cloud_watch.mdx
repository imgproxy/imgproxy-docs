---
description: Learn about how to send imgproxy metrics to Amazon CloudWatch
---
...
* `CacheHits`: the number of cache hits (sent only when greater than 0)
* `CacheMisses`: the number of cache misses (sent only when greater than 0)
* `SrcCacheHits`: the number of source image cache hits (sent only when greater than 0)
* `SrcCacheMisses`: the number of source image cache misses (sent only when greater than 0)
