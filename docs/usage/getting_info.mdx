...
```imgproxy_url_option
classify_objects:%top_k
co:%top_k
```

When `top_k` is greater than zero, imgproxy will classify the image and return the top K classes with the highest confidence scores. Read the [object classification](../features/object_classification.mdx) manual to learn how to configure object classification.
