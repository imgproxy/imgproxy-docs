...
* [`IMGPROXY_PRESERVE_HDR`]: when `true`, imgproxy will preserve 16-bit images as 16-bit. When `false`, imgproxy will convert 16-bit images to 8-bit. Colorspace (color/grayscale) is always preserved regardless of this setting. See the [Colorspace and HDR preservation](../image_formats_support.mdx#colorspace-and-hdr-preservation) guide for more details. Default: `false`
...
### Cache storage: Local filesystem {#cache-storage-local-filesystem}
...
### Cache storage: Amazon S3 {#cache-storage-amazon-s3}
...
### Cache storage: Google Cloud Storage {#cache-storage-google-cloud-storage}
...
### Cache storage: Azure Blob Storage {#cache-storage-azure-blob-storage}
...
### Cache storage: OpenStack Swift {#cache-storage-openstack-swift}
