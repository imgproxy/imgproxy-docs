...
* [`IMGPROXY_MAX_ML_CONCURRENCY`]: ((pro)) the maximum number of inference requests that can be processed concurrently by each machine learning model. When the limit is reached, additional inference requests will be queued until the previous ones are finished. Default: the number of CPU cores available to imgproxy

:::tip
The larger the `IMGPROXY_MAX_ML_CONCURRENCY` value, the more memory ML features will consume. If you don't have a lot of RAM, you may want to set it to a value lower than the number of CPU cores.
:::
