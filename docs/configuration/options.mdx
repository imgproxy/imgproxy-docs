...
* [`IMGPROXY_ALLOW_BYPASS_CACHE`]: ((pro)) when `true`, allows usage of the [bypass_cache](../usage/processing.mdx#bypass-cache) option. Default: `false`.

  :::warning
  `IMGPROXY_ALLOW_BYPASS_CACHE` allows bypassing your cache, opening the possibility of DoS attacks. Don't set it to `true` unless you are completely sure that an attacker can't change your imgproxy URLs.
  :::
...
imgproxy Pro can process digital camera RAW images. Since loading RAW images is pretty resource intensive, their processing is disabled by default. You can enable it with the following variable:

* [`IMGPROXY_ENABLE_RAW_FORMATS`]: ((pro)) when `true`, enables RAW image formats support. Default: `false`
...
* [`IMGPROXY_OBJECT_DETECTION_INFERENCE_NUM_STREAMS`]: ((pro)) when set, overrides the [IMGPROXY_ML_NUM_STREAMS](#IMGPROXY_ML_NUM_STREAMS) value for the object detection model.
...
* [`IMGPROXY_OBJECT_DETECTION_EXECUTION_MODE`]: ((pro)) when set, overrides the [IMGPROXY_ML_EXECUTION_MODE](#IMGPROXY_ML_EXECUTION_MODE) value for the object detection model.
* [`IMGPROXY_OBJECT_DETECTION_CPU_PINNING`]: ((pro)) when set, overrides the [IMGPROXY_ML_CPU_PINNING](#IMGPROXY_ML_CPU_PINNING) value for the object detection model.
...
* [`IMGPROXY_CLASSIFICATION_INFERENCE_NUM_STREAMS`]: ((pro)) when set, overrides the [IMGPROXY_ML_NUM_STREAMS](#IMGPROXY_ML_NUM_STREAMS) value for the classification model.
...
* [`IMGPROXY_CLASSIFICATION_EXECUTION_MODE`]: ((pro)) when set, overrides the [IMGPROXY_ML_EXECUTION_MODE](#IMGPROXY_ML_EXECUTION_MODE) value for the classification model.
* [`IMGPROXY_CLASSIFICATION_CPU_PINNING`]: ((pro)) when set, overrides the [IMGPROXY_ML_CPU_PINNING](#IMGPROXY_ML_CPU_PINNING) value for the classification model.
...
* [`IMGPROXY_CACHE_TAGS_FORMAT`]: ((pro)) defines how the cache tags added by the [cache_tags](../usage/processing.mdx#cache-tags) option are written to the response headers. Supported values are:
  ...
  * `cloudfront`: imgproxy will write cache tags in the [CloudFront](https://aws.amazon.com/blogs/networking-and-content-delivery/manage-caches-with-precision-using-amazon-cloudfront-invalidation-by-cache-tag/) format (`x-amz-meta-cache-tag` header)
  * `cloudflare`: imgproxy will write cache tags in the [Cloudflare](https://developers.cloudflare.com/cache/how-to/purge-cache/purge-by-tags/) format (`Cache-Tag` header)
  * `none`: imgproxy will not write cache tags to the response headers
...
* [`IMGPROXY_S3_ACCESS_POINTS`]: a semicolon-separated list of `name=arn` pairs, where `name` is an arbitrary access point name and `arn` is the access point ARN. When the name is used as a bucket name in a `s3://...` source URL, imgproxy will use the corresponding ARN to access the bucket. Default: blank
